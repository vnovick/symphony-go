...
describe('extractSubagents', () => {
  function makeEntry(event: string, message: string, sessionId?: string): IssueLogEntry {
    return { event, message, level: 'INFO', tool: '', time: '', sessionId } as unknown as IssueLogEntry;
  }
...
describe('filterByRun', () => {
  function makeEntry(event: string, message: string, opts?: { sessionId?: string; time?: string }): IssueLogEntry {
    return { event, message, level: 'INFO', tool: '', time: opts?.time ?? '', sessionId: opts?.sessionId } as unknown as IssueLogEntry;
  }
  ...
  it('includes orchestrator messages by timestamp when no sessionId on entry', () => {
    const logs = [
      makeEntry('text', 'orch msg', { time: '2026-01-01T00:02:00Z' }),
    ];
    const result = filterByRun(logs, baseRun);
    expect(result).toHaveLength(1);
  });
