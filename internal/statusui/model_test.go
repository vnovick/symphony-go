...
// ---------------------------------------------------------------------------
// buildToolStats — must aggregate both claude and codex action lines
// ---------------------------------------------------------------------------
...
func TestBuildToolStats_ClaudeActions(t *testing.T) {
	...
	assert.NotEqual(t, -1, bashIdx)
	assert.Equal(t, 2, stats[bashIdx].count)
}
...
func TestBuildToolStats_CodexActions(t *testing.T) {
	...
	assert.NotEqual(t, -1, shellIdx)
	assert.Equal(t, 2, stats[shellIdx].count)
}
...
func TestBuildToolStats_CodexResultError(t *testing.T) {
	...
	shellIdx := indexByName(stats, "shell")
	assert.NotEqual(t, -1, shellIdx)
	assert.Equal(t, 1, stats[shellIdx].count)
	assert.Equal(t, 1, stats[shellIdx].failCount)
}
...
func TestBuildToolStats_ClaudeResultError(t *testing.T) {
	...
	bashIdx := indexByName(stats, "Bash")
	assert.NotEqual(t, -1, bashIdx)
	assert.Equal(t, 1, stats[bashIdx].failCount)
}

// ---------------------------------------------------------------------------
// buildToolCalls — per-tool history for both backends
// ---------------------------------------------------------------------------
...
func TestBuildToolCalls_ClaudeActions(t *testing.T) {
	...
	assert.Equal(t, "ls", calls[0].desc)
	assert.Equal(t, "pwd", calls[1].desc)
...
func TestBuildToolCalls_CodexActions(t *testing.T) {
	...
	assert.Len(t, calls, 2)
	assert.Equal(t, "make test", calls[0].desc)
	assert.Equal(t, "make build", calls[1].desc)
}
...
func TestBuildToolCalls_MixedBackendsFiltersByName(t *testing.T) {
	...
	shellCalls := buildToolCalls(lines, "shell")
	assert.Len(t, shellCalls, 1)
}
...
func TestBuildToolCalls_CodexFailedCall(t *testing.T) {
	...
	assert.True(t, calls[0].failed)
	assert.NotEmpty(t, calls[0].errMsg)
}
...
func indexByName(stats []toolStat, name string) int {
	for i, s := range stats {
		if s.name == name {
			return i
		}
...
func toolNames(stats []toolStat) []string {
...
// ---------------------------------------------------------------------------
// buildToolStats — action_detail lines must not double-count tool calls
// (regression for Bug #3: action_detail matched generic action prefix)
// ---------------------------------------------------------------------------
...
func TestBuildToolStats_ActionDetailNotCounted(t *testing.T) {
	...
	assert.Equal(t, "Bash", stats[0].name)
...
func TestBuildToolStats_CodexActionDetailNotCounted(t *testing.T) {
	...
	stats := buildToolStats(lines)
	assert.Len(t, stats, 1)
	assert.Equal(t, "shell", stats[0].name)
	assert.Equal(t, 1, stats[0].count)
}
