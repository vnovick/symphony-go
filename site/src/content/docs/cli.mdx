...
> **Note (post-v0.2):** the `-demo` flag was removed in favour of an embedded
> quickstart template. To explore itervox without a real tracker, drop a
> `WORKFLOW.md` with `tracker.kind: memory` in your project (the template
> ships in `internal/templates/quickstart.md`) and run the daemon as usual.
