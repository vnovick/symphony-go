...
GitHub Issues does not expose custom states, but you can narrow which issues get dispatched by combining `active_states` with label filters in your prompt. Alternatively, create a project board and configure Itervox to only dispatch issues assigned to a specific column — use the web UI to pause individual issues while you refine your workflow.
