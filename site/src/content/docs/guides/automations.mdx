...
Fires when the agent encounters a rate-limit error (detected via `rate_limit_error_patterns` in the agent config or built-in heuristics).
...
- `pr_opened` and `rate_limited` trigger variables are not yet available in Liquid templates (the struct fields exist but are not rendered)
