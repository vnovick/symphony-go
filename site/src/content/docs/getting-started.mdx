...
```bash
brew tap vnovick/tap && brew install symphony-go
```

Or download the latest release from [GitHub Releases](https://github.com/vnovick/symphony-go/releases).

> **Note:** The binary installs as `symphony` and will be renamed to `itervox` in an upcoming release.
...
Create a `.symphony/.env` file (gitignored) with your API keys:
