...
| Field | Type | Default | Description |
|---|---|---|---|
| `root` | string | `~/.symphony/workspaces` | Directory where per-issue workspaces are created. Supports `~` expansion and `$VAR_NAME` env var references. |
...
The `available_models` field stores the list of models available for each backend. This is auto-populated by `symphony init` (which queries `claude --list-models` / `codex --list-models`) and used by the web dashboard's profile editor to suggest models in the dropdown.
...
```bash
symphony refresh-models --workflow WORKFLOW.md
```
...
1. `.symphony/.env` (relative to the current working directory)
2. `.env` (relative to the current working directory)

Only the first file found is loaded. Existing environment variables (already set in the shell) are never overwritten. The `.symphony/.env` file is created and git-ignored automatically by `itervox init`.

A typical `.symphony/.env` looks like:
